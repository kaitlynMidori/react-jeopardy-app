...
useEffect(() => {
  ...
  return (
    ...
    <div className="boardContainer" >
        <div className= "host"><img src={hostImage} alt="alexTrebek"/></div>
        
        <div className= "rowAndColumn">
        <div className="row categoryRow">
            {categories.map((cat)=>{
            return(
            <CategoryCard title={cat.title} key={cat.id}>{cat.text}</CategoryCard>)})}
        </div>
        <div className="row 200Row">
            {twoHundredQuestions.map((question)=>{
                return <QuestionCard level='200' question={twoHundredQuestions} 
                categoryID={question.category_id} index={twoHundredQuestions.indexOf(question)}  playersScores={props.playersScores} updateScore={props.updateScore}></QuestionCard>
            })}
        </div>
        ...
        <div className="row 600Row">
            {sixHundredQuestions.map((question)=>{
                return <QuestionCard level='600' question={sixHundredQuestions} 
                categoryID={question.category_id} index={sixHundredQuestions.indexOf(question)} playersScores={props.playersScores} updateScore={props.updateScore}></QuestionCard>
            })}
        </div>
        <div className="row 800Row">
            {eightHundredQuestions.map((question)=>{
                return <QuestionCard level='800' question={eightHundredQuestions} 
                categoryID={question.category_id} index={eightHundredQuestions.indexOf(question)} playersScores={props.playersScores} updateScore={props.updateScore}></QuestionCard>
            })}
        </div>
        <div className="row 1000Row">
            {thousandQuestions.map((question)=>{
                return <QuestionCard level='1000' question={thousandQuestions} 
                categoryID={question.category_id} index={thousandQuestions.indexOf(question)} playersScores={props.playersScores} updateScore={props.updateScore}></QuestionCard>
            })}
        </div>
        </div>
    </div>
