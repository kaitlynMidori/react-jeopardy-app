...
const [dataAvailable, setDataAvailable] = useState(false);
  ...
  function loadLeaderboard() {
    {returnedArray.map((score, index) => {
      return <LeaderboardRow key={index} playerData={playerData} player={score.name} score={score.score} rank={index+1}/> })}
  }
...
​
    <Table responsive>
      ...
      {/* {returnedArray.length === 10 ? returnedArray.map((score, i) => {
         return (
         <LeaderboardRow key={i} playerData={playerData} player={score.name} score={score.score} rank={i+1}/> }) : null)}
